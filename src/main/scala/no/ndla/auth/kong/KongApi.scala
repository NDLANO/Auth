...
object KongApi extends StrictLogging {
    val KONG_HOSTNAME = "kong" // In etc hosts when linking containers.
    val KONG_ADMIN_PORT ="8001"
    val USERNAME_PREFIX = "ndla-" // We can not use a valid uuid as username because of kong api /consumers/{username or id} where is uuid.
