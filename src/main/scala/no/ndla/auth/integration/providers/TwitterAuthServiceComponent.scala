...
import no.ndla.auth.AuthProperties.{Domain, TwitterApiKey, TwitterClientSecret}
...
trait TwitterAuthServiceComponent {
  ...
  class TwitterAuthService {
    ...
    val CallbackUrlKey = "callback_url"
    val UserInfoUrlKey = "user_info_url"
    val ClientSecretKey = "client_secret"

    val environment = Map(
      CallbackUrlKey -> s"$Domain/auth/login/twitter/verify",
      UserInfoUrlKey -> "https://api.twitter.com/1.1/account/verify_credentials.json",
      ApiKeyKey -> TwitterApiKey,
      ClientSecretKey -> TwitterClientSecret
    )

    val service: OAuthService = new ServiceBuilder()
      .provider(new TwitterApi)
      .apiKey(environment(ApiKeyKey))
      .apiSecret(environment(ClientSecretKey))
      .callback(environment(CallbackUrlKey))
      .build()
    ...
    def getRedirectUri(successUrl: String, failureUlr: String): String = {
      ...
      val redirectService: OAuthService = new ServiceBuilder()
        ...
        .apiKey(environment(ApiKeyKey))
        .apiSecret(environment(ClientSecretKey))
        .callback(environment(CallbackUrlKey).addParams(params.toList))
        .build()
    ...
    private def getUser(accessToken: Token): NdlaUser = {
      val request: OAuthRequest = new OAuthRequest(Verb.GET, environment(UserInfoUrlKey), service)
